...
import {Row, Col, Modal, Select, TreeSelect, Form, Table,Input} from 'antd';
    ...
    render() {
        ...
        return (
            <Modal title="调客" confirmLoading={showLoading} className='adjustCustomer' maskClosable={false} visible={showAdjustCustomer} onOk={this.handleOk} onCancel={this.handleCancel}>
                <Table columns={this.getCustomerColumns()} dataSource={activeCustomers} bordered size="middle" pagination={false} scroll={{y:300}}/>
                <Row>
                    {
                        orgList.length > 0 ?
                            <Col span={24} pull={1}>
                ...
                <Row>
                    <Col span={24} pull={1}>
                        <FormItem {...formItemLayout} label={(<span>调客原因</span>)}>
                            {getFieldDecorator('desc', {
                                rules: [{required: true, message: '请输入调客原因!'}],
                            })(
...
function mapStateToProps(state) {
    return {
        ...
        userList: state.orgData.userList,
