...
import {changeKeyWord, searchCustomer, setLoadingVisible, saveSearchCondition, getRepeatGroup} from '../actions/actionCreator';
...
class SearchBox extends Component {
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.searchInfo.activeMenu === 'menu_public_pool') {
            this.setState({searchType: '0'});
            this.handleSearchTypeChange('0');
        }
    }
    //搜索处理
    handleSearch = () => {
        ...
        if (standardCondition.isOnlyRepeat) {
            this.props.dispatch(getRepeatGroup(standardCondition));
        } else {
    ...
    selectBefore = () => {
        ...
        return (
            activeMenu !== "menu_public_pool" ? <Select value={this.state.searchType} style={{width: 80}} onChange={this.handleSearchTypeChange}>
        ...
        return (
            <div>
                <div className="searchBox">
                    <p>
                        <Input style={{width: '450px'}} addonBefore={this.selectBefore()} onPressEnter={this.handleSearch} placeholder={this.state.searchType === '1' ? '请输入业务员姓名或电话号码' : '请输入客户姓名或电话号码'} value={keyword} onChange={this.handleKeyWordChange} />
...
export function formatSearchCondition(condition) {
    ...
    if (standardCondition) {
        ...
        standardCondition.keyWord = standardCondition.keyWord.trim();
