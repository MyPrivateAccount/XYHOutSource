...
const initState = {
    ...
    searchResult: {extension: [], pageIndex: 0, pageSize: 10, totalCount: 0, repeatCount: 0},//搜索结果
    ...
    isLoading: false, // 无效客户确认loading
    isShow: false, // 拉无效的弹框
    currentCustomer: {},//点击拉无效的当前客户信息
    type: 'list',
    repeatGroups: {extension: [], pageIndex: 0, pageSize: 10, totalCount: 0},//重客分组
    repeatCustomers: [],//重客列表
};
let reducerMap = {};
...
reducerMap[actionTypes.SEARCH_CUSTOMER_COMPLETE] = function (state, action) {
    ...
    if (!action.payload) {
        result = {extension: [], pageIndex: 0, pageSize: 10, totalCount: 0, repeatCount: 0};
    } else {
        ...
        result.repeatCount = (result.pageIndex && result.pageIndex > 0) ? state.searchResult.repeatCount : 0;
    }
...
reducerMap[actionTypes.SHOW_CUSTOMER_LOSS_MODAL] = function (state, action) {
    let currentCustomer = {...action.payload}
    return Object.assign({}, state, {isShow: true, currentCustomer: currentCustomer, type: currentCustomer.type});
}
reducerMap[actionTypes.HIDE_CUSTOMER_LOSS_MODAL] = function (state, action) {
    return Object.assign({}, state, {isShow: false});
}
reducerMap[actionTypes.CUSTOMER_LOSS_START] = function (state, action) {
    return Object.assign({}, state, {isLoading: true});
}
reducerMap[actionTypes.CUSTOMER_LOSS_END] = function (state, action) {
    ...
    return Object.assign({}, state, {isLoading: false, isShow: false});
}
// reducerMap[actionTypes.CUSTOMER_LOSS_ACTIVE_END] = function (state, action) {
//     let res = action.payload.data;
//     if(res.code === '0') {
//         notification.success({
//             message: '激活成功',
...
//获取重客分组完成
reducerMap[actionTypes.GET_REAPEAT_GROUP_COMPLETE] = function (state, action) {
    return Object.assign({}, state, {repeatGroups: action.payload});
}
//根据分组名获取重客完成
reducerMap[actionTypes.GET_CUSTOMER_BY_GROUP_COMPLETE] = function (state, action) {
    return Object.assign({}, state, {repeatCustomers: action.payload});
}

reducerMap[actionTypes.GET_REPEAT_COUNT_COMPLETE] = function (state, action) {
    let searchResult = {...state.searchResult};
    searchResult.repeatCount = Math.abs(searchResult.totalCount * 1 - action.payload * 1);
