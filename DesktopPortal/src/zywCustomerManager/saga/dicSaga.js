...
///获取意向业态数据
export function* getTreeListAsync(state) {
    let result = { isOk: false, extension: [], msg: '系统参数获取失败！' };
    let url = WebApiConfig.dic.GetTreeList;
    try {
        let res = yield call(ApiClient.get, url)
        console.log(res, '获取TreeList参数列表');
        getApiResult(res, result);
        if (result.isOk) {
            yield put({ type: actionUtils.getActionType(actionTypes.GET_TREE_LIST_COMPLETE), payload: result.extension });
        }
    } catch (e) {
...
export default function* watchDicAllAsync() {
    ...
    yield takeLatest(actionUtils.getActionType(actionTypes.GET_TREE_LIST), getTreeListAsync);
