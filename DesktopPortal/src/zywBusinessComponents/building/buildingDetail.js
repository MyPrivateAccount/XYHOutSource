...
class BuildingDetail extends Component {
    ...
    render() {
        ...
        const shopSaleStatus = (basicData.find(d => d.groupId === 'ZYW_SHOP_SALE_STATUS') || {}).dicPars || [];
                    ...
                    <Content className='content buildingDish'>
                        ...
                        <Row style={{marginTop: '25px'}}>
                            <Col className="content">
                                <Radio.Group value={this.state.activeTab} size='large' onChange={(e) => this.handleChangTab(e.target.value)}>
                                    ...
                                    <Radio.Button value="marketingCtrl">租控看板</Radio.Button>
                        ...
                        }
                        {
                            this.state.activeTab === "shopLayout" ? <div className="searchResult" style={{padding: "20px 50px"}}>
                                ...
                                {buildingShops.map((item, i) => <ShopResultItem key={i} shopInfo={item} basicData={basicData} buildingInfo={this.props.buildingInfo} gotoShopDetail={() => this.gotoShopDetail(item.id)} />)}</div> : null
                        }
                    </Content>
                </Layout>
            </div>
