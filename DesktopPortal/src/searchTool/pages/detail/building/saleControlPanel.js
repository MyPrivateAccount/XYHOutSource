...
import { Layout, Menu, Select, Spin, Collapse, Popover } from 'antd'
...
class SaleControlPanel extends Component {
    ...
    componentWillMount() {
        ...
        if (this.props.shopSaleStatus) {
            this.props.shopSaleStatus.map(status => {
    ...
    getPieChartData() {
        let activeBuildingShops = this.props.activeBuildingShops;
        let seriesData = [];
        this.state.shopSaleStatus.map(status => {
            let dicCode = this.props.shopSaleStatus.find(statusDic => statusDic.key === status).value;
            let result = activeBuildingShops.filter(shop => shop.saleStatus === dicCode);
    ...
    render() {
        ...
        activeBuildingShops.forEach(v => {
            ...
            v.backgroundColor = this.props.shopSaleStatus.find(s => s.value === v.saleStatus).ext1;
