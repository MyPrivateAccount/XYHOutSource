...
import SkComponent from './sKComponet'
...
class SKPanel extends Component {
    state = {
        loading: false,
        factGet: {},
        report: {},
    ...
    submit = () => {
        ...
        let errors = validations.validate(values, reportValidation.sk);
    ...
    _submit = async (values) => {
        ...
        if (r.code === '0') {
            ...
            let yjfp = newFact.report.reportYjfp;

            
            this.setState({ factGet: { ...factGet }, report: {...this.state.report, reportYjfp: yjfp} })
            notification.success({message:'收款申请已提交'})
        } else {
...
export default connect(MapStateToProps, MapDispatchToProps)(SKPanel);