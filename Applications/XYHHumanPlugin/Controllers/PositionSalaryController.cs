...
namespace XYHHumanPlugin.Controllers
{
    ...
    public class PositionSalaryController : Controller
    {
        ...
        public async Task<ResponseMessage> HumanPositionSalaryCallback([FromBody] ExamineResponse examineResponse)
        {
            ...
            {
                response.Code = ResponseCodeDefines.ModelStateInvalid;
                Logger.Trace($"新增职位薪酬信息回调(HumanPositionSalaryCallback)模型验证失败：\r\n{response.Message ?? ""}，\r\n请求参数为：\r\n" + (examineResponse != null ? JsonHelper.ToJson(examineResponse) : ""));
                return response;
            }
            try
            {
